...
function UserLoginForm() {
  ...
  const LoginValidationSchema = object({
    ...
    password: z
      .string({ required_error: "Password can't be blank!" })
    ...
    <>
      <Formik
        ...
        onSubmit={async (values) => {
          ...
          try {
            let regularCallbackUrl = callbackUrl.includes("/auth/logout")
              ? "/"
              : callbackUrl;
            ...
            if (res && !res?.error) {
              ...
              setErrMsg(`Login error: Internal Server Error`);
              console.error("Login error ->", res?.error);
            }
          } catch (err: any) {
            setErrMsg(`Login error: ${err.message}`);
          }
        }}
      >
            ...
            <div className="p-4">
              ...
              <div className="py-6">
                <button
                  ...
                  className={
                    "block px-8 w-full py-2 xs:w-max border-none shadow rounded-full font-semibold text-sm text-gray-50 hover:bg-indigo-500 bg-indigo-400 cursor-pointer"
                  }
                >
