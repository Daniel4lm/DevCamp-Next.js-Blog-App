...
hljs.configure({
  languages: ["javascript", "java", "css", "php", "go", "html"],
...
const PostBody = ({
  ...
  const { data: post, isLoading, isError } = usePostQuery(postSlug);
  ...
  useEffect(() => {
    ...
    if (effectRun.current === true) {
      let codeBlocks: NodeListOf<HTMLPreElement> =
        document.querySelectorAll("#post-body pre");

      codeBlocks.forEach((block) => {
        if (navigator.clipboard) {
          let copyButton = document.createElement("button");
          copyButton.classList.add("hidden", "prism-copy-button");
          ...
          text.innerText = "Copy";
          copyButton.prepend(img);
          copyButton.append(text);
          block.appendChild(copyButton);

          block.addEventListener("mouseover", () =>
            copyButton.classList.replace("hidden", "block")
          );
          block.addEventListener("mouseout", () =>
            copyButton.classList.replace("block", "hidden")
          );
  ...
  if (isError)
    return (
      <div className="alert alert-warning text-xl text-center">
        Oppps! Post not found!
      </div>
    );
  ...
  return (
    <>
      {isLoading ? (
        <div className="text-xl text-center">Loading post...</div>
      ) : (
        <>
          <ScrollToTopButton />
          ...
          <div
            id="post-wrapper"
            className="w-full overflow-y-auto bg-white dark:bg-navbar-dark dark:text-slate-100 sm:rounded-lg border-t border-b sm:border border-[#d1d9d1] dark:border-slate-500 mb-14 py-6"
          >
            <article id="user-post" className="flex flex-col">
              <header className="mx-4 sm:mx-12 my-2">
                <h1 className="font-bold text-xl text-center lg:text-3xl my-4">
                  {post?.title}
                </h1>
                <div
                  id="post-tags"
                  className="flex flex-wrap justify-center my-2 text-xs md:text-sm"
                >
                  {post?.tags?.map((tag) => (
                    <Link
                      className="mx-[0.2em] my-[0.6em]"
                      key={tag.id}
              ...
              {post?.photo_url ? (
                <div
                  ...
                  className="relative max-w-[44rem] 2xl:max-w-3xl mx-4 sm:mx-12 md:mx-auto my-8 rounded-lg"
                >
                  <a
                    id="download-icon"
