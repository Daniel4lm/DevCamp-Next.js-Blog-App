...
function WorkMenu() {
  ...
  return (
    <>
      ...
      >
        <div
          className="cursor-pointer flex flex-col items-center justify-center text-slate-700 hover:text-[#755FFF] dark:text-gray-200 hover:underline"
          onClick={handleMenuOpen}
        >
          <WorkMenuIcon />
          <span data-test="work-menu-title" className="hidden xs:block">
            My work
          </span>
        </div>
        ...
        <MemoizedSideMenu
          id="work-menu"
          classNames="w-full h-full shadow dark:text-white border-l border-slate-300 dark:border-slate-600 bg-white dark:bg-menu-dark-github overflow-auto"
          isOpen={menuOpen}
          side="right"
          closeFunc={closeMenu}
        >
          ...
          >
            ...
            <SmartLink href="/" onClick={closeMenu}>
              <li
                id="home-icon"
                className="px-4 py-2 flex items-center gap-2 rounded-md hover:bg-indigo-50 dark:hover:bg-slate-600 hover:text-black text-gray-800 dark:text-gray-200"
              >
                <HomeIcon />
                <span>Home</span>
              </li>
