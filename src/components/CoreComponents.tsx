  ...
  return (
    <Link href={link} className={`${linkClass}`}>
      {!src || src === "undefined" ? (
        <AvatarIcon
          styleClass={
            "rounded-full w-full h-full object-cover object-center p-[1px] bg-white dark:bg-slate-400 border border-gray-300 dark:border-slate-400"
          }
        />
      ) : (
        <Image
          ...
          src={
            (src !== "undefined" ? src : defaultAvatar.src) || defaultAvatar.src
          }
          width={60}
          height={60}
          priority={true}
...
function AvatarLink({ withLink, ...props }: AvatarProps) {
    ...
    if (!props.src || props.src === "undefined") {
      ...
      return (
        <Image
          ...
          width={60}
          height={60}
          className={`rounded-full object-cover object-center bg-white dark:bg-slate-400 border border-gray-300 ${props.className}`}
        />
      );
    }
  };
  ...
  return (
    <>
      {withLink ? (
        <Link href={withLink}>{renderIcon()}</Link>
      ) : (
        <>{renderIcon()}</>
      )}
    </>
    ...
    isOpen && isOpen !== "" ? (
      <dialog ref={modalRef}>
        <>
          ...
          >
            <div
              className={`absolute z-[400] ${style}`}
            ...
            >
              <header className="flex justify-between items-center gap-2 p-4 dark:border-slate-400 rounded-t-lg">
                ...
                <button
                  className="text-[#8F8F8F] bg-[#F4F6FA] p-2 rounded-full"
                  onClick={closeModal}
                >
                  <CloseIcon />
                </button>
...
function HeroIcon({
  ...
  return <span className={`${name} ${classNames}`} />;
}

export { AvatarLink, HeroIcon, Modal, UserAvatar };
