    ...
    return (
        <section className="flex flex-col w-full mt-5" id="editor-container">
            ...
            <div className="invisible flex flex-row items-center justify-end rounded-b-lg px-5 py-2 z-100 editor-footer">
                <a className="flex m-auto text-sm text-indigo-400 dark:!text-slate-300" href="#" id="show-more"></a>
                <div className="flex justify-self-end text-slate-500 dark:!text-slate-400 text-xs" id="word-count"></div>
            </div>
        </section>
    )
}

let setupMceEditor = (editor: HTMLDivElement | null, toolbar: HTMLDivElement | null, content: HTMLDivElement | null) => {
    // Avoid error when navigating away from the page while loading editor
    if (!editor) return

    let footer = document.querySelector('.editor-footer') as HTMLDivElement | null
    ...
    return {
        ...
        setup: (editor: TinyMCEEditor) => {
            editor.on('init', () => {
                ...
                let editorEl = document.querySelector('.tox-tinymce') as HTMLDivElement | null
                let editorContentEl = document.querySelector('.tox-tinymce') as HTMLDivElement | null
                let toolbar = document.querySelector('.tox-editor-header') as HTMLDivElement | null
                editor.dom.setAttrib(editor.dom.select('main'), 'data-testid', 'tinymce-editor')
            ...
            editor.on("ResizeEditor", () => {
                let editor = document.querySelector('.tox-tinymce') as HTMLDivElement | null
            ...
            editor.on("focus", function (e) {
                ...
                let editorEl = document.querySelector('.tox-tinymce') as HTMLDivElement | null
                editorEl?.classList.add('!ring-2', 'ring-indigo-400', 'ring-opacity-90', '!border-transparent', 'dark:border-transparent', 'dark:ring-blue-400')
            ...
            editor.on("blur", function () {
                let editorEl = document.querySelector('.tox-tinymce') as HTMLDivElement | null
                editorEl?.classList.remove('!ring-2', 'ring-indigo-400', 'ring-opacity-90', '!border-transparent', 'dark:border-transparent', 'dark:ring-blue-400')
